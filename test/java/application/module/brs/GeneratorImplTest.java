...
public class GeneratorImplTest {
    ...
    private static final byte[] exampleGenSig = Convert
            .parseHexString("6ec823b5fd86c4aee9f7c3453cacaf4a43296f48ede77e70060ca8225c2855d0");
    ...
    public void setUpGeneratorTest() {
        ...
        FluxCapacitor fluxCapacitorLnTime = QuickMocker.fluxCapacitorEnabledFunctionalities(FluxValues.POC2,
                FluxValues.SODIUM);
        FluxCapacitor fluxCapacitorPocPlus = QuickMocker.fluxCapacitorEnabledFunctionalities(FluxValues.POC2,
                FluxValues.POC_PLUS);
    ...
    public void testGeneratorCalculateGenerationSignature() {
        byte[] genSig = generator.calculateGenerationSignature(exampleGenSig,
    ...
    public void testGeneratorCalculateDeadline() {
        BigInteger hit = generator.calculateHit(TestConstants.TEST_ACCOUNT_NUMERIC_ID_PARSED, 0, exampleGenSig,
                generator.calculateScoop(exampleGenSig, exampleHeight), exampleHeight);
    ...
    public void testGeneratorCalculateSodiumDeadline() {
        BigInteger hit = generatorLnTime.calculateHit(TestConstants.TEST_ACCOUNT_NUMERIC_ID_PARSED, 0, exampleGenSig,
                generator.calculateScoop(exampleGenSig, exampleHeight), exampleHeight);
    ...
    public void testGeneratorCalculateNextDeadline() {
        BigInteger hit = generatorPocPlus.calculateHit(TestConstants.TEST_ACCOUNT_NUMERIC_ID_PARSED, 0, exampleGenSig,
                generator.calculateScoop(exampleGenSig, exampleHeight), exampleHeight);
        BigInteger deadline = generatorPocPlus.calculateDeadline(hit, exampleBaseTarget, 100 * exampleAverageCommitment,
                exampleAverageCommitment, exampleHeight);
        assertEquals(BigInteger.valueOf(1205L), deadline);
    }

    @Test
    public void testGeneratorCalculateHit() {
        assertEquals(new BigInteger("14592422770739690569"), generator
                .calculateHit(TestConstants.TEST_ACCOUNT_NUMERIC_ID_PARSED, 0, exampleGenSig, 0, exampleHeight));
        // Scoop data is the generation signature repeated - not intended to be acutal
        // scoop data for the purpose of this test. It is twice as long as the gensig as
        // this is the expected scoop size.
        assertEquals(new BigInteger("16142911724569013009"),
                generator.calculateHit(exampleGenSig, Convert.parseHexString(
