...
public class SqlAccountStore implements AccountStore {
    ...
    public SqlAccountStore(DerivedTableManager derivedTableManager, DBCacheManagerImpl dbCacheManager) {
        rewardRecipientAssignmentTable = new VersionedEntitySqlTable<Account.RewardRecipientAssignment>(
                "reward_recip_assign", brs.schema.Tables.REWARD_RECIP_ASSIGN, rewardRecipientAssignmentDbKeyFactory,
                derivedTableManager) {
            ...
            public void trim(int height) {
                logger.debug("Trimming reward_recip_assign table to height {}", height);
                Db.useDSLContext((Consumer<DSLContext>) ctx -> {
                    int batchSize = Constants.TRIM_BATCH_SIZE;
                    int totalDeleted = 0;
                    int deletedRows;
                    do {
                        deletedRows = ctx.deleteFrom(REWARD_RECIP_ASSIGN)
                                .where(REWARD_RECIP_ASSIGN.HEIGHT.le(height))
                                .and(REWARD_RECIP_ASSIGN.LATEST.isFalse())
                                .limit(batchSize)
                                .execute();
        ...
        accountAssetTable = new VersionedEntitySqlTable<Account.AccountAsset>("account_asset",
                brs.schema.Tables.ACCOUNT_ASSET, accountAssetDbKeyFactory, derivedTableManager) {
        ...
        accountTable = new VersionedBatchEntitySqlTable<Account>("account", brs.schema.Tables.ACCOUNT,
        ...
        accountBalanceTable = new VersionedBatchEntitySqlTable<Account.Balance>("account_balance",
                brs.schema.Tables.ACCOUNT_BALANCE, accountBalanceDbKeyFactory, derivedTableManager, dbCacheManager,
