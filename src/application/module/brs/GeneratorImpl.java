...
import application.module.brs.crypto.Crypto;
...
public class GeneratorImpl implements Generator {
    ...
    public GeneratorImpl(Blockchain blockchain, DownloadCacheImpl downloadCache, AccountService accountService,
            TimeService timeService, FluxCapacitor fluxCapacitor) {
    ...
    public void generateForBlockchainProcessor(ThreadPool threadPool, BlockchainProcessor blockchainProcessor) {
        threadPool.scheduleThread("GenerateBlocks", generateBlockThread(blockchainProcessor), 500,
    ...
    public GeneratorState addNonce(String secretPhrase, Long nonce, byte[] publicKey) {
        ...
        if (curGen == null || generator.getBlock() > curGen.getBlock()
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Account {} started mining, deadline {} seconds", Convert.toUnsignedLong(id),
    ...
    @Override
    public BigInteger calculateDeadline(BigInteger hit, long capacityBaseTarget, long commitment,
            long averageCommitment, int blockHeight) {
        ...
        if (fluxCapacitor.getValue(FluxValues.POC_PLUS, blockHeight)) {
            // private static final double lnScale = 49d; // value that would keep the
            // legacy network size estimation close to real capacity
            ...
            if (deadline.bitLength() < 100 && deadline.longValue() > 0L) {
                // Avoid the double precision limit for extremely large numbers (of no value)
                // and zero logarithm
    ...
    public class GeneratorStateImpl implements GeneratorState {
        ...
        private GeneratorStateImpl(String secretPhrase, Long nonce, byte[] publicKey, Long account) {
            ...
            // need to store publicKey in addition to accountId, because the account may not
            // have had its publicKey set yet
            ...
            deadline = calculateDeadline(hit, baseTarget, commitmment, lastBlock.getAverageCommitment(),
    ...
    public static class MockGenerator extends GeneratorImpl {
        ...
        public MockGenerator(PropertyService propertyService, Blockchain blockchain, AccountService accountService,
                TimeService timeService, FluxCapacitor fluxCapacitor) {
    ...
    public long estimateCommitment(long generatorId, Block previousBlock) {
        // Check on the number of blocks mined to estimate the capacity and also the
        // committed balance
        int nBlocksMined = 0;
        ...
        // We also check if the blockchain actually have blockIt, we might be processing
        // a fork
        while (blockIt != null && (endHeight >= blockchain.getHeight() ||
            ...
            if (blockIt.getGeneratorId() == generatorId) {
                if (height - endHeight <= Constants.CAPACITY_ESTIMATION_BLOCKS)
                    ...
                    if (fluxCapacitor.getValue(FluxValues.SPEEDWAY, height)
                            && height - endHeight <= Constants.CAPACITY_ESTIMATION_BLOCKS_MID) {
            ...
            for (Transaction tx : blockIt.getTransactions()) {
                if (tx.getSenderId() == generatorId) {
                    if (blockIt.getHeight() <= height - commitmentWait
                            && tx.getType() == TransactionType.SignaMining.COMMITMENT_ADD) {
        ...
        long estimatedCapacityGb = genesisTarget * nBlocksMined * 1000L
                / (capacityBaseTarget * capacityEstimationBlocks);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Block {}, Network {} TiB, ID {}, forged {}/{} blocks, {} TiB, committedAmountNQT {}, commitmentNQT {}",
                    height,
