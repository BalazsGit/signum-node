...
import application.module.brs.Escrow.Decision;
import application.module.brs.Escrow.DecisionType;
import application.module.brs.db.SignumKey;
import application.module.brs.db.SignumKey.LongKeyFactory;
import application.module.brs.db.VersionedEntityTable;
import application.module.brs.db.sql.DbKey.LinkKeyFactory;
import application.module.brs.db.store.EscrowStore;
...
import static application.module.brs.schema.Tables.ESCROW;
...
public class EscrowServiceImpl implements EscrowService {
    ...
    @Override
    public void addEscrowTransaction(Account sender, Account recipient, Long id, Long amountNQT, int requiredSigners,
            Collection<Long> signers, int deadline, DecisionType deadlineAction) {
        final SignumKey dbKey = escrowDbKeyFactory.newKey(id);
        Escrow newEscrowTransaction = new Escrow(dbKey, sender, recipient, id, amountNQT, requiredSigners, deadline,
                deadlineAction);
    ...
    public DecisionType checkComplete(Escrow escrow) {
        ...
        Decision recipientDecision = decisionTable
                .get(decisionDbKeyFactory.newKey(escrow.getId(), escrow.getRecipientId()));
    ...
    public void updateOnBlock(Block block, int blockchainHeight) {
        ...
        escrowTable.getManyBy(getUpdateOnBlockClause(block.getTimestamp()), 0, -1)
    ...
    public synchronized void doPayout(DecisionType result, Block block, int blockchainHeight, Escrow escrow) {
        switch (result) {
            case RELEASE:
                accountService.addToBalanceAndUnconfirmedBalanceNQT(accountService.getAccount(escrow.getRecipientId()),
                        escrow.getAmountNQT());
                saveResultTransaction(block, escrow.getId(), escrow.getRecipientId(), escrow.getAmountNQT(),
                        DecisionType.RELEASE, blockchainHeight);
                break;
            case REFUND:
                accountService.addToBalanceAndUnconfirmedBalanceNQT(accountService.getAccount(escrow.getSenderId()),
                        escrow.getAmountNQT());
                saveResultTransaction(block, escrow.getId(), escrow.getSenderId(), escrow.getAmountNQT(),
                        DecisionType.REFUND, blockchainHeight);
                break;
            case SPLIT:
                ...
                saveResultTransaction(block, escrow.getId(), escrow.getRecipientId(), halfAmountNQT, DecisionType.SPLIT,
                        blockchainHeight);
                saveResultTransaction(block, escrow.getId(), escrow.getSenderId(),
                        escrow.getAmountNQT() - halfAmountNQT, DecisionType.SPLIT, blockchainHeight);
                break;
