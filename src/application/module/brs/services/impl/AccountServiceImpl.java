...
import application.module.brs.Account.Event;
...
import application.module.brs.db.SignumKey.LinkKeyFactory;
...
import static application.module.brs.schema.Tables.ACCOUNT;
...
public class AccountServiceImpl implements AccountService {
    ...
    public Account getAccount(byte[] publicKey) {
        ...
        throw new RuntimeException("DUPLICATE KEY for account " + Convert.toUnsignedLong(account.getId())
                + " existing key " + Convert.toHexString(account.getPublicKey()) + " new key "
                + Convert.toHexString(publicKey));
    }

    @Override
    public CollectionWithIndex<AssetTransfer> getAssetTransfers(long accountId, int from, int to) {
        return new CollectionWithIndex<AssetTransfer>(assetTransferStore.getAccountAssetTransfers(accountId, from, to),
                from, to);
    }
    ...
    public void setRewardRecipientAssignment(Account account, Long recipient) {
        ...
        if (assignment == null) {
            ...
            assignment = new RewardRecipientAssignment(account.getId(), account.getId(), recipient,
                    (int) (currentHeight + Constants.SIGNA_REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME), signumKey);
        } else {
            assignment.setRecipient(recipient,
                    (int) (currentHeight + Constants.SIGNA_REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME));
        }
