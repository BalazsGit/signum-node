...
import application.module.brs.DigitalGoodsStore.Event;
import application.module.brs.DigitalGoodsStore.Goods;
import application.module.brs.DigitalGoodsStore.Purchase;
...
public class DGSGoodsStoreServiceImpl implements DGSGoodsStoreService {
    ...
    public DGSGoodsStoreServiceImpl(Blockchain blockchain, DigitalGoodsStoreStore digitalGoodsStoreStore,
    ...
    public void purchase(Transaction transaction, Attachment.DigitalGoodsPurchase attachment) {
        ...
        if (!goods.isDelisted() && attachment.getQuantity() <= goods.getQuantity()
            ...
            accountService.addToUnconfirmedBalanceNQT(buyer,
                    Convert.safeMultiply(attachment.getQuantity(), attachment.getPriceNqt()));
            // restoring the unconfirmed balance if purchase not successful, however buyer
            // still lost the transaction fees
        }
    }
    ...
    public void deliver(Transaction transaction, Attachment.DigitalGoodsDelivery attachment) {
        ...
        accountService.addToBalanceAndUnconfirmedBalanceNQT(seller,
                Convert.safeSubtract(totalWithoutDiscount, attachment.getDiscountNqt()));
