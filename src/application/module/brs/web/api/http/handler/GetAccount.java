...
public final class GetAccount extends ApiServlet.JsonRequestHandler {
    ...
    public GetAccount(ParameterService parameterService, AccountService accountService, Blockchain blockchain,
            Generator generator) {
        super(new LegacyDocTag[] { LegacyDocTag.ACCOUNTS }, ACCOUNT_PARAMETER, HEIGHT_PARAMETER,
                GET_COMMITTED_AMOUNT_PARAMETER, ESTIMATE_COMMITMENT_PARAMETER);
    ...
    protected JsonElement processRequest(HttpServletRequest req) throws SignumException {
        ...
        if (account.getPublicKey() != null) {
            if (!Convert.checkAllZero(account.getPublicKey())) {
                response.addProperty(ACCOUNT_RESPONSE + "RSExtended",
                        SignumCrypto.getInstance().getAddressFromPublic(account.getPublicKey()).getExtendedAddress());
            }
