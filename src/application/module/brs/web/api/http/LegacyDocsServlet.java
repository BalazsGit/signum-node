package application.module.brs.web.api.http;

import application.module.brs.util.Convert;
import application.module.brs.util.Subnet;
...
public class LegacyDocsServlet extends HttpServlet {
    ...
    private static final String HEADER_1_a = "<!DOCTYPE html>\n"
            + "<html>\n"
            + "<head>\n"
            + "    <meta charset=\"UTF-8\"/>\n"
    ...
    private static final String HEADER_1_b = "    <link href=\"/legacy_api_doc_assets/bootstrap.min.css\" rel=\"stylesheet\" type=\"text/css\" />"
    ...
    private static final String HEADER_1_c = "       </div>"
    ...
    private static final String HEADER_2 = "    </ul>"
            + "  </div>"
            + "  <div  class=\"col-xs-8 col-sm-9 col-md-10\">"
            + "    <div class=\"panel-group\" id=\"accordion\">";

    private static final String FOOTER_1 = "    </div> "
            + "  </div> "
            + "</div> "
            + "</div> "
            + "<script>"
    ...
    private static final String FOOTER_2 = "    $(\".collapse-link\").click(function(event) {"
        ...
        try {
            try (PrintWriter writer = resp.getWriter()) {
                ...
                if (requestHandler != null) {
                    writer.print(form(requestType, true, requestHandler.getClass().getName(),
                            requestHandler.getParameters(), requestHandler.requirePost()));
    ...
    private String form(String requestType, boolean singleView, String className, List<String> parameters,
            boolean requirePost) {
        ...
        if (!singleView) {
            ...
            buf.append(
                    "\" target=\"_blank\" style=\"font-weight:normal;font-size:14px;color:#777;\"><span class=\"glyphicon glyphicon-new-window\"></span></a>");
            buf.append(" &nbsp;&nbsp;");
        }
        buf.append("</span>");
        buf.append("</h4>");
