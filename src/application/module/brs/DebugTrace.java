...
public final class DebugTrace {
    ...
    private boolean include(Attachment attachment) {
        if (attachment instanceof Attachment.DigitalGoodsPurchase) {
            long sellerId = dgsGoodsStoreService.getGoods(((Attachment.DigitalGoodsPurchase) attachment).getGoodsId())
                    .getSellerId();
            return include(sellerId);
        } else if (attachment instanceof Attachment.DigitalGoodsDelivery) {
            long buyerId = dgsGoodsStoreService
                    .getPurchase(((Attachment.DigitalGoodsDelivery) attachment).getPurchaseId()).getBuyerId();
            return include(buyerId);
        } else if (attachment instanceof Attachment.DigitalGoodsRefund) {
            long buyerId = dgsGoodsStoreService
                    .getPurchase(((Attachment.DigitalGoodsRefund) attachment).getPurchaseId()).getBuyerId();
            return include(buyerId);
        }
        return false;
    }
