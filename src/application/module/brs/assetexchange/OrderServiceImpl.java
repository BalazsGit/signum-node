...
import application.module.brs.Attachment.ColoredCoinsOrderCancellation;
import application.module.brs.Attachment.ColoredCoinsOrderPlacement;
import application.module.brs.Order.Ask;
import application.module.brs.Order.Bid;
import application.module.brs.Order.OrderJournal;
...
import application.module.brs.db.store.OrderStore;
...
class OrderServiceImpl {
    ...
    public CollectionWithIndex<Ask> getAskOrdersByAccountAsset(final long accountId, final long assetId, int from,
            int to) {
        return new CollectionWithIndex<Ask>(orderStore.getAskOrdersByAccountAsset(accountId, assetId, from, to), from,
                to);
    }
    ...
    public CollectionWithIndex<Bid> getBidOrdersByAccountAsset(final long accountId, final long assetId, int from,
            int to) {
        return new CollectionWithIndex<Bid>(orderStore.getBidOrdersByAccountAsset(accountId, assetId, from, to), from,
                to);
    }
    ...
    public CollectionWithIndex<OrderJournal> getTradeJournal(final long accountId, final long assetId, int from,
        ...
        CollectionWithIndex<OrderJournal> ordersWithIndex = new CollectionWithIndex<Order.OrderJournal>(orders,
                transactions.size() == to - from + 1 ? to + 1 : -1);
        ...
        for (Transaction transaction : transactions) {
            Attachment.ColoredCoinsOrderCancellation cancellation = (ColoredCoinsOrderCancellation) transaction
                    .getAttachment();
    ...
    private void matchOrders(long assetId) {
        ...
        while ((askOrder = getNextAskOrder(assetId)) != null
            ...
            accountService.addToBalanceAndUnconfirmedBalanceNQT(askAccount,
            ...
            bidOrderUpdateQuantityQNT(bidOrder,
                    Convert.safeSubtract(bidOrder.getQuantityQNT(), trade.getQuantityQNT()));
            ...
            accountService.addToBalanceNQT(bidAccount,
                    -Convert.safeMultiply(trade.getQuantityQNT(), trade.getPriceNQT()));
            accountService.addToUnconfirmedBalanceNQT(bidAccount,
