...
import application.module.brs.DigitalGoodsStore;
...
import application.module.brs.db.VersionedValuesTable;
import application.module.brs.db.store.DerivedTableManager;
import application.module.brs.db.store.DigitalGoodsStoreStore;
...
public class SqlDigitalGoodsStoreStore implements DigitalGoodsStoreStore {
    ...
    private static final DbKey.LongKeyFactory<DigitalGoodsStore.Purchase> feedbackDbKeyFactory = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>(
            PURCHASE.ID) {
        @Override
    ...
    private final SignumKey.LongKeyFactory<DigitalGoodsStore.Purchase> purchaseDbKeyFactory = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>(
            PURCHASE.ID) {
        @Override
        public SignumKey newKey(DigitalGoodsStore.Purchase purchase) {
            return purchase.dbKey;
    ...
    public SqlDigitalGoodsStoreStore(DerivedTableManager derivedTableManager) {
        purchaseTable = new VersionedEntitySqlTable<DigitalGoodsStore.Purchase>("purchase",
                application.module.brs.schema.Tables.PURCHASE,
        ...
        feedbackTable = new VersionedValuesSqlTable<DigitalGoodsStore.Purchase, EncryptedData>("purchase_feedback",
                application.module.brs.schema.Tables.PURCHASE_FEEDBACK, feedbackDbKeyFactory, derivedTableManager) {
            ...
            protected void save(DSLContext ctx, DigitalGoodsStore.Purchase purchase, EncryptedData encryptedData) {
                ...
                ctx.insertInto(
                        ...
                        PURCHASE_FEEDBACK.HEIGHT, PURCHASE_FEEDBACK.LATEST).values(
                                purchase.getId(),
                                data, nonce,
                                application.module.brs.Signum.getBlockchain().getHeight(), true)
                        .execute();
            }
        };

        publicFeedbackTable = new VersionedValuesSqlTable<DigitalGoodsStore.Purchase, String>(
                "purchase_public_feedback", application.module.brs.schema.Tables.PURCHASE_PUBLIC_FEEDBACK,
        ...
        goodsTable = new VersionedEntitySqlTable<DigitalGoodsStore.Goods>("goods",
                application.module.brs.schema.Tables.GOODS,
            ...
            protected List<SortField<?>> defaultSort() {
                List<SortField<?>> sort = new ArrayList<>();
                sort.add(application.module.brs.schema.Tables.GOODS.field("timestamp", Integer.class).desc());
                sort.add(application.module.brs.schema.Tables.GOODS.field("id", Long.class).asc());
                return sort;
            }
        };
    }
    ...
    private void savePurchase(DSLContext ctx, DigitalGoodsStore.Purchase purchase) {
        ...
        ctx.insertInto(PURCHASE,
                ...
                .set(PURCHASE.HAS_FEEDBACK_NOTES,
                        purchase.getFeedbackNotes() != null && !purchase.getFeedbackNotes().isEmpty())
    ...
    @Override
    public Collection<DigitalGoodsStore.Goods> getSellerGoods(final long sellerId, final boolean inStockOnly, int from,
            int to) {
        List<SortField<?>> sort = new ArrayList<>();
    ...
    @Override
    public Collection<DigitalGoodsStore.Purchase> getSellerBuyerPurchases(final long sellerId, final long buyerId,
            int from, int to) {
