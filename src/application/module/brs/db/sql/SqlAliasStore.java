...
import application.module.brs.db.store.AliasStore;
...
import static application.module.brs.schema.Tables.ALIAS;
import static application.module.brs.schema.Tables.ALIAS_OFFER;

public class SqlAliasStore implements AliasStore {

    private static final DbKey.LongKeyFactory<Alias.Offer> offerDbKeyFactory = new DbKey.LongKeyFactory<Alias.Offer>(
            ALIAS_OFFER.ID) {
        @Override
    ...
    public SqlAliasStore(DerivedTableManager derivedTableManager) {
        ...
        aliasTable = new VersionedEntitySqlTable<Alias>("alias", application.module.brs.schema.Tables.ALIAS,
                aliasDbKeyFactory,
                derivedTableManager) {
            @Override
    ...
    private class SqlOffer extends Alias.Offer {
        private SqlOffer(Record record) {
            super(record.get(ALIAS_OFFER.ID), record.get(ALIAS_OFFER.PRICE),
    ...
    private void saveAlias(DSLContext ctx, Alias alias) {
        ctx.insertInto(ALIAS).set(ALIAS.ID, alias.getId()).set(ALIAS.ACCOUNT_ID, alias.getAccountId())
    ...
    public Collection<Alias> getTLDs(int from, int to) {
        return aliasTable.getManyBy(application.module.brs.schema.Tables.ALIAS.TLD.isNull(), from, to);
    }
    ...
    public Alias getTLD(String tldName) {
        return aliasTable.getBy(
                application.module.brs.schema.Tables.ALIAS.ALIAS_NAME_LOWER.eq(tldName.toLowerCase(Locale.ENGLISH))
                        .and(ALIAS.TLD.isNull()));
    }

    @Override
    public Alias getTLD(long tldId) {
        return aliasTable.getBy(application.module.brs.schema.Tables.ALIAS.ID.eq(tldId).and(ALIAS.TLD.isNull()));
    }

    @Override
    public Alias getAlias(String aliasName, long tld) {
        return aliasTable.getBy(
                application.module.brs.schema.Tables.ALIAS.ALIAS_NAME_LOWER.eq(aliasName.toLowerCase(Locale.ENGLISH))
                        .and(ALIAS.TLD.eq(tld)));
    }
