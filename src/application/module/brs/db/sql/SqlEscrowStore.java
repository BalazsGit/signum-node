...
import static application.module.brs.schema.Tables.ESCROW_DECISION;
...
public class SqlEscrowStore implements EscrowStore {
    ...
    private final DbKey.LinkKeyFactory<Escrow.Decision> decisionDbKeyFactory = new DbKey.LinkKeyFactory<Escrow.Decision>(
    ...
    public SqlEscrowStore(DerivedTableManager derivedTableManager) {
        escrowTable = new VersionedEntitySqlTable<Escrow>("escrow", application.module.brs.schema.Tables.ESCROW,
                escrowDbKeyFactory,
                derivedTableManager) {
            @Override
        ...
        decisionTable = new VersionedEntitySqlTable<Escrow.Decision>("escrow_decision",
                application.module.brs.schema.Tables.ESCROW_DECISION, decisionDbKeyFactory, derivedTableManager) {
            @Override
    ...
    private class SqlDecision extends Escrow.Decision {
        private SqlDecision(Record record) {
            super(decisionDbKeyFactory.newKey(record.get(ESCROW_DECISION.ESCROW_ID),
                    record.get(ESCROW_DECISION.ACCOUNT_ID)), record.get(ESCROW_DECISION.ESCROW_ID),
