...
import application.module.brs.db.VersionedBatchEntityTable;
import application.module.brs.db.VersionedEntityTable;
import application.module.brs.db.cache.DBCacheManagerImpl;
...
import application.module.brs.schema.tables.records.AccountBalanceRecord;
...
public class SqlAccountStore implements AccountStore {
    ...
    private static final DbKey.LongKeyFactory<Account> accountDbKeyFactory = new DbKey.LongKeyFactory<Account>(
            ACCOUNT.ID) {
        @Override
    ...
    private static final DbKey.LongKeyFactory<Account.Balance> accountBalanceDbKeyFactory = new DbKey.LongKeyFactory<Account.Balance>(
            ACCOUNT_BALANCE.ID) {
        @Override
    ...
    private static final DbKey.LongKeyFactory<Account.RewardRecipientAssignment> rewardRecipientAssignmentDbKeyFactory = new DbKey.LongKeyFactory<Account.RewardRecipientAssignment>(
    ...
    private static final DbKey.LinkKeyFactory<Account.AccountAsset> accountAssetDbKeyFactory = new DbKey.LinkKeyFactory<Account.AccountAsset>(
            "account_id", "asset_id") {
        @Override
    ...
    public SqlAccountStore(DerivedTableManager derivedTableManager, DBCacheManagerImpl dbCacheManager) {
        rewardRecipientAssignmentTable = new VersionedEntitySqlTable<Account.RewardRecipientAssignment>(
                "reward_recip_assign", application.module.brs.schema.Tables.REWARD_RECIP_ASSIGN,
        ...
        accountAssetTable = new VersionedEntitySqlTable<Account.AccountAsset>("account_asset",
                application.module.brs.schema.Tables.ACCOUNT_ASSET, accountAssetDbKeyFactory, derivedTableManager) {
        ...
        accountTable = new VersionedBatchEntitySqlTable<Account>("account",
                application.module.brs.schema.Tables.ACCOUNT,
                ...
                for (Account account : accounts) {
                    ...
                    accountQueries.add(
                            ctx.insertInto(ACCOUNT)
                                    .columns(ACCOUNT.ID, ACCOUNT.HEIGHT, ACCOUNT.CREATION_HEIGHT, ACCOUNT.PUBLIC_KEY,
                                            ACCOUNT.KEY_HEIGHT,
                                            ACCOUNT.NAME, ACCOUNT.DESCRIPTION, ACCOUNT.LATEST)
                                    .values(account.getId(), height, account.getCreationHeight(),
                                            account.getPublicKey(), account.getKeyHeight(),
        ...
        accountBalanceTable = new VersionedBatchEntitySqlTable<Account.Balance>("account_balance",
                application.module.brs.schema.Tables.ACCOUNT_BALANCE, accountBalanceDbKeyFactory, derivedTableManager,
                dbCacheManager,
    ...
    public int getAssetAccountsCount(Asset asset, long minimumQuantity, boolean ignoreTreasury, boolean unconfirmed) {
        return Db.fetchWithDSLContext(ctx -> {
            ...
            if (minimumQuantity > 0L) {
                select = select.and((unconfirmed ? ACCOUNT_ASSET.UNCONFIRMED_QUANTITY : ACCOUNT_ASSET.QUANTITY)
                        .ge(minimumQuantity));
            }
            if (ignoreTreasury) {
                ...
                if (transaction != null) {
                    List<Long> ignoredAccounts = ctx.select(TRANSACTION.RECIPIENT_ID).from(TRANSACTION)
                            ...
                            .and(TRANSACTION.REFERENCED_TRANSACTION_FULLHASH
                                    .eq(Convert.parseHexString(transaction.getFullHash())))
    ...
    public Account.AccountAsset getAccountAsset(Long accountId, Long assetId) {
        return getAccountAssetTable()
                .getBy(ACCOUNT_ASSET.ACCOUNT_ID.eq(accountId).and(ACCOUNT_ASSET.ASSET_ID.eq(assetId)));
    }

    @Override
    public Collection<Account.AccountAsset> getAssetAccounts(Asset asset, boolean ignoreTreasury, long minimumQuantity,
        ...
        if (minimumQuantity > 0L) {
            condition = condition.and(
                    (unconfirmed ? ACCOUNT_ASSET.UNCONFIRMED_QUANTITY : ACCOUNT_ASSET.QUANTITY).ge(minimumQuantity));
        }
    ...
    public boolean setOrVerify(Account acc, byte[] key, int height) {
        if (acc.getPublicKey() == null) {
            ...
            if (logger.isInfoEnabled()) {
                ...
                logger.info(
                        "Account key for {} was already set to a different one at the same height, current height is {}, rejecting new key",
                        Convert.toUnsignedLong(acc.id), height);
            }
            return false;
            ...
            if (Db.isInTransaction()) {
                if (logger.isInfoEnabled()) {
                    logger.info(
                            ...
                            Convert.toUnsignedLong(acc.id), height, acc.getKeyHeight());
                }
                acc.setPublicKey(key);
        ...
        if (logger.isInfoEnabled()) {
            logger.info("DUPLICATE KEY!!!");
            logger.info("Invalid key for account {} at height {}, was already set to a different one at height {}",
                    Convert.toUnsignedLong(acc.id), height, acc.getKeyHeight());
        }
        return false;
    }
