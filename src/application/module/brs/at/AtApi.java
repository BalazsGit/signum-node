...
interface AtApi {
    ...
    // note: these 8 math ops are intended for a future implementaion so no need to
    // support them
    ...
    /**
     * divides A by B (result in B) *can cause a divide by zero error which would
     * stop the machine
     *
     * @param state AT machine state
     */
    ...
    /**
     * divides B by A (result in A) *can cause a divide by zero error which would
     * stop the machine
     *
     * @param state AT machine state
     */
    ...
    /**
     * bool if the signature attached as a message on tx id A1, page A2 can be
     * verified for the [AT ID, B2..4] message.
     * 
     * @param state
    ...
    /**
     * @param state AT machine state
     * @return the map value stored with keys in A (A1=key1, A2=key2, A3=at_id
     *         (optional, if 0 use the self id))
     */
     ...
     *         random id is a 64bit signed value (always positive) and this is a
     *         blocking function
     */
