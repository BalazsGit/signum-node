...
class AtMachineProcessor {
    ...
    private int getAddr(boolean isCode) {
        ...
        fun.addr1 = (machineData.getApCode())
        ...
        if (op == OpCode.E_OP_CODE_NOP) {
            ...
            if (disassemble) {
                ...
                if (machineData.getMachineState().cs == 0)
                    ...
                    long val = machineData.getApData().getLong(machineData.getdSize() + machineData.getcCallStackBytes()
                            - machineData.getMachineState().cs * 8);
                    machineData.getMachineState().cs--;
                    int addr = (int) val;
            ...
            if (Signum.getFluxCapacitor().getValue(FluxValues.SIGNUM, machineData.getCreationBlockHeight())) {
                ...
                if (rc == 0 || disassemble) {
                    ...
                    if (rc == 0 || disassemble) {
                        ...
                        if (disassemble) {
                            if (!determineJumps && logger.isDebugEnabled())
                                logger.debug("SET @{} {}", String.format("($%8s+$%8s)", addr1, addr2).replace(' ', '0'),
                            ...
                            if (!validAddr((int) addr, false))
                                ...
                                machineData.getMachineState().pc += rc;
                                machineData.getApData().putLong((int) addr * 8,
            ...
            if (rc == 0 || disassemble) {
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled())
                        logger.debug("MOD @{} ${}", String.format("%8x", fun.addr1).replace(' ', '0'),
                                String.format("%8s", fun.addr2).replace(' ', '0'));
                } else {
            ...
            if (rc == 0 || disassemble) {
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled()) {
                        logger.debug("POW @{} ${}", String.format("%8x", fun.addr1).replace(' ', '0'),
            ...
            if (rc == 0 || disassemble) {
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled()) {
                        logger.debug("MDV @{} ${}", String.format("%8x", fun.addr1).replace(' ', '0'),
                                String.format("%8x", fun.addr2).replace(' ', '0'));
                    }
                } else {
                    machineData.getMachineState().pc += rc;
                    ...
                    if (den != 0L) {
                        try {
                            BigInteger bigResult = BigInteger.valueOf(x).multiply(BigInteger.valueOf(y))
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled()) {
                        if (op == OpCode.E_OP_CODE_SHL_DAT)
                            logger.debug("SHL @{} ${}", String.format("%8x", fun.addr1).replace(' ', '0'),
                                    String.format("%8x", fun.addr2).replace(' ', '0'));
                        else
                            logger.debug("SHR @{} ${}", String.format("%8x", fun.addr1).replace(' ', '0'),
                                    String.format("%8x", fun.addr2).replace(' ', '0'));
                    }
                } else {
                    machineData.getMachineState().pc += rc;
            ...
            if (rc == 0 || disassemble) {
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled())
                        logger.debug("FUN {} ${} ${}", fun.fun, String.format("%8x", fun.addr3).replace(' ', '0'),
                                String.format("%8x", fun.addr2).replace(' ', '0'));
                } else {
                    machineData.getMachineState().pc += rc;
            ...
            if (rc == 0) {
                ...
                if (disassemble) {
                    if (!determineJumps && logger.isDebugEnabled()) {
                        logger.debug("FUN @{} {} ${}", String.format("%8x", fun.addr3).replace(' ', '0'), fun.fun,
                                String.format("%8x", fun.addr2).replace(' ', '0'));
            ...
            getAddr(true); // rico666: Why getAddr if rc is set hard anyway ?? // TODO check if this
                           // updates the buffer or can be removed
