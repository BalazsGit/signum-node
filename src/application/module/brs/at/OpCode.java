...
final class OpCode {
    ...
    static final short CHECK_HASH160_A_WITH_B = 0x0203; // EXT_FUN_RET @addr to bool if RIPEMD160 hash of A1..3 matches
                                                        // B1..3
    ...
    static final short CHECK_SIG_B_WITH_A = 0x0206; // EXT_FUN_RET @addr to bool if [AT ID, B2..4] signature can be
                                                    // verified with the message attached on tx id in A1 (page in A2)
                                                    // for account id in A3

    static final short GET_BLOCK_TIMESTAMP = 0x0300; // EXT_FUN_RET sets @addr to the timestamp of the current block
    static final short GET_CREATION_TIMESTAMP = 0x0301; // EXT_FUN_RET sets @addr to the timestamp of the AT creation
                                                        // block
    static final short GET_LAST_BLOCK_TIMESTAMP = 0x0302; // EXT_FUN_RET sets @addr to the timestamp of the previous
                                                          // block
    static final short PUT_LAST_BLOCK_HASH_IN_A = 0x0303; // EXT_FUN puts the block hash of the previous block in A
    static final short A_TO_TX_AFTER_TIMESTAMP = 0x0304; // EXT_FUN_DAT sets A to tx hash of the first tx after $addr
                                                         // timestamp
    ...
    static final short GET_TIMESTAMP_FOR_TX_IN_A = 0x0307; // EXT_FUN_RET if A is a valid tx then @addr to the tx
                                                           // timestamp
    static final short GET_RANDOM_ID_FOR_TX_IN_A = 0x0308; // EXT_FUN_RET if A is a valid tx then @addr to the tx random
                                                           // id***
    ...
    static final short B_TO_ADDRESS_OF_CREATOR = 0x030b; // EXT_FUN sets B1 to the address of the AT's creator (or the
                                                         // creator of the AT id on B2 if B2!=0)
    static final short GET_CODE_HASH_ID = 0x030c; // EXT_FUN_RET sets @addr to the code hash ID of the AT's (or of the
                                                  // AT id on B2 if B2!=0)
    static final short B_TO_ASSET_IDS_OF_TX_IN_A = 0x030d; // EXT_FUN if A is a valid tx then B set to the asset ids
                                                           // received (terminated by 0)

    static final short GET_CURRENT_BALANCE = 0x0400; // EXT_FUN_RET sets @addr to current balance of the AT for the
                                                     // asset id in B2 (or SIGNA if B2==0)
    static final short GET_PREVIOUS_BALANCE = 0x0401; // EXT_FUN_RET sets @addr to the balance it had last had when
                                                      // running*
    static final short SEND_TO_ADDRESS_IN_B = 0x0402; // EXT_FUN_DAT if B is a valid address then send it $addr amount**
    static final short SEND_ALL_TO_ADDRESS_IN_B = 0x0403; // EXT_FUN if B is a valid address then send it the entire
                                                          // balance
    static final short SEND_OLD_TO_ADDRESS_IN_B = 0x0404; // EXT_FUN if B is a valid address then send it the old
                                                          // balance**
    static final short SEND_A_TO_ADDRESS_IN_B = 0x0405; // EXT_FUN if B is a valid address then send it A as a message
    static final short ADD_MINUTES_TO_TIMESTAMP = 0x0406; // EXT_FUN_RET_DAT_2 set @addr1 to timestamp $addr2 plus
                                                          // $addr3 minutes***

    static final short GET_MAP_VALUE_KEYS_IN_A = 0x0407; // EXT_FUN_RET keys in A1, A2, and A3 (if A3==0 use the AT ID
                                                         // as key3)
    static final short SET_MAP_VALUE_KEYS_IN_A = 0x0408; // EXT_FUN keys in A1 and A2 with value in A4
    static final short ISSUE_ASSET = 0x0409; // EXT_FUN_RET issue asset with name in A and dec. places in B1, returns
                                             // the asset id
    static final short MINT_ASSET = 0x040a; // EXT_FUN mint B1 quantity of asset ID in B2
    static final short DIST_TO_ASSET_HOLDERS = 0x040b; // EXT_FUN B1 min holding of asset ID in B2, A1 the signa amount
                                                       // to distribute, A3 the assetId to distribute, A4 the asset
                                                       // quantity to distribute
    static final short GET_ASSET_HOLDERS_COUNT = 0x040c; // EXT_FUN_RET number of tokens holders with more than B1
                                                         // holdings of asset ID in B2
    static final short GET_ACTIVATION_FEE = 0x040d; // EXT_FUN_RET sets @addr to the activation fee of this AT (or the
                                                    // AT id on B2 if B2!=0)
    static final short PUT_LAST_BLOCK_GSIG_IN_A = 0x040e; // EXT_FUN puts the block generation signature of the previous
                                                          // block in A
    static final short GET_ASSET_CIRCULATING = 0x040f; // EXT_FUN_RET total circulating supply of tokens of asset ID in
                                                       // B2
    static final short GET_ACCOUNT_BALANCE = 0x0410; // EXT_FUN_RET sets @addr to current balance of the account id in
                                                     // B1 for the asset id in B2 (or SIGNA if B2==0)
