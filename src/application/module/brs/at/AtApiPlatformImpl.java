...
import application.module.brs.Appendix;
import application.module.brs.Asset;
import application.module.brs.Attachment;
import application.module.brs.Attachment.ColoredCoinsAssetTransfer;
...
public class AtApiPlatformImpl extends AtApiImpl {
    ...
    public void aToTxAfterTimestamp(long val, AtMachineState state) {
        ...
        byte[] b = state.getId();
        logger.debug("aToTxAfterTimestamp, startHeight: {}, endHeight: {}, numOfTx: {}, minAct: {}", height,
                state.getHeight(), numOfTx, state.minActivationAmount());
        long tx = findTransaction(height, state.getHeight(), AtApiHelper.getLong(b), numOfTx,
        ...
        if (state.getVersion() > 2) {
            ...
            if (assetId != 0) {
                if (tx.getAttachment() instanceof Attachment.ColoredCoinsAssetTransfer) {
                    ...
                    Attachment.ColoredCoinsAssetMultiTransfer assetTransfer = (Attachment.ColoredCoinsAssetMultiTransfer) tx
                            .getAttachment();
    ...
    public void bToAssetsOfTxInA(AtMachineState state) {
        ...
        if (tx.getAttachment() instanceof Attachment.ColoredCoinsAssetTransfer) {
            Attachment.ColoredCoinsAssetTransfer assetTransfer = (Attachment.ColoredCoinsAssetTransfer) tx
                    .getAttachment();
    ...
    public void sendToAddressInB(long val, AtMachineState state) {
        ...
        AtTransaction tx = new AtTransaction(TransactionType.Payment.ORDINARY, state.getId(), state.getB1().clone(),
                val, null);
        logger.debug(
    ...
    public long issueAsset(AtMachineState state) {
        ...
        AtTransaction tx = new AtTransaction(TransactionType.ColoredCoins.ASSET_ISSUANCE, state.getId(), null, 0L, 0L,
                decimals, b.array());
        state.addTransaction(tx);
    ...
    public void sendAllToAddressInB(AtMachineState state) {
        AtTransaction tx = new AtTransaction(TransactionType.AutomatedTransactions.AT_PAYMENT, state.getId(),
                state.getB1().clone(), state.getgBalance(), null);
        state.addTransaction(tx);
    ...
    public void sendOldToAddressInB(AtMachineState state) {
        if (state.getpBalance() > state.getgBalance()) {
            AtTransaction tx = new AtTransaction(TransactionType.AutomatedTransactions.AT_PAYMENT, state.getId(),
                    state.getB1(), state.getgBalance(), null);
            state.addTransaction(tx);
            ...
            AtTransaction tx = new AtTransaction(TransactionType.AutomatedTransactions.AT_PAYMENT, state.getId(),
                    state.getB1(), state.getpBalance(), null);
            state.addTransaction(tx);
