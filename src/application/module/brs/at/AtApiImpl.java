    ...
    public long checkHash160AWithB(AtMachineState state) {
        if (Signum.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_3)) {
            ...
            return (ripemdb.getLong(0) == AtApiHelper.getLong(state.getB1()) &&
                    ...
                    ripemdb.getInt(16) == ((int) (AtApiHelper.getLong(state.getB3()) & 0x00000000FFFFFFFFL))) ? 1 : 0;
        } else {
            return (Arrays.equals(state.getA1(), state.getB1()) &&
                    Arrays.equals(state.getA2(), state.getB2()) &&
                    (AtApiHelper.getLong(state.getA3()) & 0x00000000FFFFFFFFL) == (AtApiHelper.getLong(state.getB3())
                            & 0x00000000FFFFFFFFL)) ? 1 : 0;
        }
    }
