package application.module.node.statistics;
...
public class StatisticsManagerImpl {
    ...
    public void blockAdded() {
        if (addedBlockCount++ == 0) {
            ...
            if (logger.isInfoEnabled()) {
                final String handleText = "handling {} blocks/s" + cacheStatistics.values().stream()
                        .map(cacheInfo -> " " + cacheInfo.getCacheInfoAndReset()).collect(Collectors.joining());
    ...
    private class CacheStatisticsOverview {
        ...
        private String getCacheInfoAndReset() {
            ...
            final Float totalHitRatio = (totalCacheHits + totalCacheMisses) > 0
            ...
            return String.format("%s cache hit ratio now/total:%.2f%%/%.2f%%", cacheName, hitRatio * 100,
                    totalHitRatio * 100);
        }
