...
import static application.module.node.schema.Tables.ALIAS_OFFER;
...
public class SqlAliasStore implements AliasStore {
    ...
    public Collection<Alias> getTLDs(int from, int to) {
        return aliasTable.getManyBy(application.module.node.schema.Tables.ALIAS.TLD.isNull(), from, to);
    }
    ...
    public Alias getTLD(String tldName) {
        return aliasTable.getBy(
                application.module.node.schema.Tables.ALIAS.ALIAS_NAME_LOWER.eq(tldName.toLowerCase(Locale.ENGLISH))
                        .and(ALIAS.TLD.isNull()));
    }

    @Override
    public Alias getTLD(long tldId) {
        return aliasTable.getBy(application.module.node.schema.Tables.ALIAS.ID.eq(tldId).and(ALIAS.TLD.isNull()));
    }

    @Override
    public Alias getAlias(String aliasName, long tld) {
        return aliasTable.getBy(
                application.module.node.schema.Tables.ALIAS.ALIAS_NAME_LOWER.eq(aliasName.toLowerCase(Locale.ENGLISH))
