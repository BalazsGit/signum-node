package application.module.node.web.api.ws;

import application.module.node.*;
import application.module.node.props.Props;
import application.module.node.web.api.ws.common.Debouncer;
import application.module.node.web.api.ws.common.SimpleScheduler;
import application.module.node.web.api.ws.emitter.data.ConnectedEventData;
import application.module.node.web.api.ws.emitter.*;
...
public class BlockchainEventNotifier {
    ...
    private BlockchainEventNotifier(WebServerContext context) {
        ...
        this.context.getBlockchainProcessor().addListener(this::onBlockPushedEvent,
                BlockchainProcessor.Event.BLOCK_PUSHED);
        this.context.getTransactionProcessor().addListener(this::onPendingTransactionEvent,
    ...
    private void initializeHeartBeat() {
        ...
        if (intervalSecs >= blockTimeSecs) {
            ...
            logger.warn("Heartbeat interval must be less than block time ({} seconds) - set to {} seconds",
                    blockTimeSecs, intervalSecs);
        }
        heartbeat = new SimpleScheduler(intervalSecs, SHUTDOWN_TIMEOUT_SECS, () -> withActiveConnectionsOnly(
        ...
        withActiveConnectionsOnly(
                () -> blockPushedDebouncer.debounce(
                        () -> withHeartbeatPaused(
                                () -> sendToAll(
                                        connection -> {
                                            int currentHeight = context.getBlockchainProcessor()
    ...
    private void onPendingTransactionEvent(List<? extends Transaction> transactions) {
        withActiveConnectionsOnly(
                () -> withHeartbeatPaused(
                        () -> sendToAll(
                                connection -> new PendingTransactionsAddedEventEmitter(connection)
                                        .emit(transactions))));
    }
    ...
    public void shutdown() {
        ...
        if (heartbeat != null) {
            try {
                ...
                logger.error("Error shutting down heartbeat scheduler", t);
        ...
        try {
            ...
            logger.error("Error shutting down block pushed debouncer", t);
            if (firstException == null) {
                firstException = t;
            }
        }
        try {
            ...
            logger.error("Error shutting down notify executor", t);
            if (firstException == null) {
                firstException = t;
            }
        }
        if (firstException != null) {
            ...
            throw new RuntimeException("Graceful WebSocket shutdown not successful", firstException);
        }
    }
